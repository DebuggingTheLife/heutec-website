...
const productMenu = ["LED Beleuchtung", "Outdoor Power Stations", "Smart Lighting", "Solar & Außenbeleuchtung"];
...
const solutionCards: CardItem[] = [
  { title: "LED Beleuchtung für Unternehmen", description: "Energieeffiziente LED-Leuchten für Hallen, Büros, Lagerflächen, Verkaufsräume und Außenbereiche." },
...
const whyUs = ["Umweltfreundliche Qualitätsprodukte", "Erfahrung mit LED-Beleuchtung und Energiespeichern", "Großhandelsfähigkeit und Projektkompetenz", "Alles aus einer Hand: Produkte, Planung, Finanzierung und Umsetzung", "Flexible Lösungen für Handel und Gewerbe", "Zuverlässige Kommunikation und langfristige Partnerschaft"];
...
const faqs = [
  { question: "Bieten Sie nur Produkte oder auch komplette Projekte an?", answer: "Wir bieten beides: LED- und Energiespeicherprodukte im Großhandel sowie komplette Projektlösungen von Planung bis Installation." },
  ...
  { question: "Welche Produkte liefern Sie im Großhandel?", answer: "LED-Leuchten, technische Beleuchtung, dekorative Lichtlösungen, Solar- und Außenbeleuchtung sowie Outdoor Power Stations." },
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur">
      <div className="section-shell flex h-24 items-center justify-between gap-5">
        <a href="#" className="flex items-center gap-3" aria-label="Startseite">
          ...
          <span><span className="block text-xl font-semibold tracking-tight">HeuTec GmbH</span><span className="block text-xs uppercase tracking-[0.22em] text-slate-500">LED · Energy · Projects</span></span>
        </a>
  ...
  return (
    <section className="hero-section">
      ...
      <div className="section-shell relative z-10 flex min-h-[calc(100vh-96px)] items-center py-20 text-white"><div className="max-w-4xl">
        ...
        <h1 className="mt-6 text-5xl font-semibold leading-tight tracking-tight sm:text-6xl lg:text-7xl">LED Beleuchtung und Energiespeicher für Unternehmen</h1>
        ...
        <ul className="mt-8 grid gap-3 text-lg font-semibold text-white">{["Bis zu 90% weniger Beleuchtungskosten", "LED Beleuchtung, Outdoor Power Stations und Projektlösungen", "Full-Service-Projektmanagement von Planung bis Installation", "Finanzierungsmöglichkeiten für gewerbliche Projekte"].map((point) => <li key={point} className="flex items-center gap-3"><span className="h-3 w-3 rounded-full bg-warm" />{point}</li>)}</ul>
...
export default function Home() {
  return <main className="min-h-screen bg-white text-ink"><Header /><Hero /><LogoCarousel />
    ...
    <section id="produkte" className="bg-mist py-20 lg:py-28"><div className="section-shell"><SectionHeader eyebrow="Produkte" title="LED Beleuchtung und Outdoor Energy Storage im Großhandel" text="Wir liefern hochwertige LED-Leuchten und portable Energiespeicher für den gewerblichen Handel. Unser Sortiment verbindet zuverlässige Qualität, modernes Design und nachhaltige Anwendungsmöglichkeiten." /><div className="mt-12 grid gap-8 lg:grid-cols-2">{productCategories.map((product) => <ProductCategoryCard key={product.title} product={product} />)}</div></div></section>
    <section className="section-shell py-20 lg:py-28"><div className="grid items-center gap-12 lg:grid-cols-[0.9fr_1.1fr]"><div><SectionHeader title="Warum moderne LED- und Energiespeicherlösungen?" /><p className="mt-6 text-lg leading-8 text-slate-600">LED reduziert Betriebskosten, Outdoor Power Stations schaffen Unabhängigkeit und Backup-Sicherheit. Kombinierte Lösungen helfen Unternehmen, Effizienz, Nachhaltigkeit und Attraktivität im Retail- und Projektgeschäft zu verbessern.</p></div><div className="grid gap-5 sm:grid-cols-2">{benefits.map((benefit) => <BenefitCard key={benefit} title={benefit} />)}</div></div></section>
